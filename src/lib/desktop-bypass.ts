// Helper used by every clock-in / clock-out fetch so the `?desktop=1` override
// works end-to-end on mobile.
//
// When the user opens ANY page with `?desktop=1`, we remember it for the rest
// of the browser session (sessionStorage) and forward it to the clock-in/out
// API in TWO ways:
//   • the `x-desktop-bypass: 1` request header, and
//   • a `?desktop=1` query param on the request URL.
...
/**
 * True when the desktop bypass is active for this session: `?desktop=1` is in
...
export function desktopBypassHeader(): Record<string, string> {
  return isDesktopBypassActive() ? { "x-desktop-bypass": "1" } : {};
}

/**
 * Append `?desktop=1` to an API URL when the bypass is active. Query strings
...
export function withDesktopBypassParam(url: string): string {
  ...
  return url + (url.includes("?") ? "&" : "?") + "desktop=1";
}

/**
 * Server-side check — true when the caller opted into the desktop bypass via
 * the `x-desktop-bypass` header. Routes ALSO check `?desktop=1` on the request
