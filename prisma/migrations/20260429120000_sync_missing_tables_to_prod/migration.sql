-- Sync missing tables from dev to prod.
-- Adds 18 tables that exist in nb_dashboard_dev but not in nb_dashboard:
--   AttendanceRegularization, AuditLog, CompOffRequest,
--   EngageComment, EngagePost, EngageReaction,
--   Expense, GoalCycle, KeyResult, Notification,
--   OnDutyRequest, Payslip, ResearcherPipelineSnapshot,
--   TeamManagerRating, TravelRequest, UserTabPermission,
--   Violation, WFHRequest
--
-- Does NOT touch any shared table (no ALTER on User, Goal, LeaveApplication, etc.)
-- and does NOT drop the prod-only hiring/onboarding/payroll tables.
...
CREATE TABLE "AttendanceRegularization" (
    ...
    "finalApprovedById" INTEGER,
    "finalApprovedAt" TIMESTAMP(3),
    "finalApprovalNote" TEXT,
...
ALTER TABLE "AttendanceRegularization" ADD CONSTRAINT "AttendanceRegularization_finalApprovedById_fkey" FOREIGN KEY ("finalApprovedById") REFERENCES "User"("id") ON DELETE SET NULL ON UPDATE CASCADE;
ALTER TABLE "AttendanceRegularization" ADD CONSTRAINT "AttendanceRegularization_grantedByAdminId_fkey" FOREIGN KEY ("grantedByAdminId") REFERENCES "User"("id") ON DELETE SET NULL ON UPDATE CASCADE;
